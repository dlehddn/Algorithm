...
public class Main3 {
    ...
    public static void main(String[] args) throws IOException {
        System.setIn(new FileInputStream("src/Samsung/메이즈러너/input.txt"));
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if (peopleMap[i][j][turn - 1][1] > 0) {
                    boolean move = false; // 이동했는지 안했는지 판단
                    for (int k = 0; k < 4; k++) {
                        int nextY = i + dy[k];
                        int nextX = j + dx[k];
                        // 출구로 나갔던지 아니던지 일단 이동한거임, 출구로 나갔으면 복사X, 못나갔으면 복사O
